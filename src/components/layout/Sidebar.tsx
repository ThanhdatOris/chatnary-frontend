...
export default function Sidebar() {
  ...
  const groupedChats = groupChatsByTime(chats.slice(0, displayLimit));
  const hasMore = chats.length > displayLimit;
  
  return (
    <aside className="flex w-64 flex-col fixed inset-y-0 left-0 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
      {/* Logo & Brand */}
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto">
          ...
          <div key={section.title} className={sectionIdx > 0 ? 'mt-6' : ''}>
            ...
            <div className="space-y-1">
              {section.items.map((item) => {
                const isActive = pathname === item.href || (item.href === '/chat' && pathname?.startsWith('/chat') && pathname !== '/chat');
        ...
        <div className="mt-6">
          <Link href="/chat">
            <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:from-blue-700 hover:to-purple-700 transition-all text-sm font-medium">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (confirm('Xóa chat này?')) {
                                chatsApi.deleteChat(chat.id).then(() => {
